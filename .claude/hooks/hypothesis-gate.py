...
def main():
    ...
    # Cuando se escribe el propio board, validamos su contenido desde stdin
    # (no puede existir en disco aún). Cuando se escribe hypotheses.md, lo
    # leemos desde disco (y exigimos que ya exista).
    if target == "experiment-board.json":
        raw = (payload.get("tool_input", {}) or {}).get("content", "") or ""
        if not raw.strip():
            allow()
        try:
            board = json.loads(raw)
        except (json.JSONDecodeError, ValueError) as exc:
            fail([f"  El JSON del experiment-board.json no es válido: {exc}"])
    else:
        # target == "hypotheses.md"
